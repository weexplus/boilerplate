    ...
    if (self.selected) {
        ...
        for(UIView *v in self.subviews)
        {
            if  ([v isKindOfClass:[UILabel class]])  {
                ((UILabel *)v).textColor = self.indicatorColor;
            }
        }
        [bezierPath stroke];
...
#pragma mark - WXViewPagerController
@interface WXViewPagerController () <UIPageViewControllerDataSource, UIPageViewControllerDelegate, UIScrollViewDelegate>
...
@implementation WXViewPagerController
...
- (void)viewDidLoad {
    [super viewDidLoad];
//    self.backView = [[UIView alloc] initWithFrame:CGRectMake(0.0, self.baseY, self.view.frame.size.width, [self.tabHeight floatValue])];
//    self.backView.backgroundColor = [UIColor whiteColor];
//    [self.view addSubview:self.backView];
//    [self.view sendSubviewToBack:self.backView];
    
    UIView *topLine=[[UIView alloc] initWithFrame:CGRectMake(0,self.baseY+[self.tabHeight floatValue]-0.5,self.view.frame.size.width,0.5)];//
    topLine.backgroundColor=[UIColor colorWithRed:204 / 255.0 green:204 / 255.0 blue:204 / 255.0 alpha:1.0];
    //        [self.view insertSubview:topLine aboveSubview:self.tabsView];
...
//    frame.origin.x = 0.0;
    ...
    frame.size.width = CGRectGetWidth(self.view.frame)-48;
    ...
    frame.origin.y = self.tabsView.frame.size.height;
...
- (void)setTabOffset:(NSNumber *)tabOffset {
    ...
    //_tabOffset = [NSNumber numberWithInt:([tabOffset intValue]+20)];
}
...
- (void)setActiveContentIndex:(NSUInteger)activeContentIndex {
    ...
    __weak WXViewPagerController *weakSelf = self;
...
- (void)defaultSetup {
    ...
    if (!self.tabsView) {
        ...
        self.tabsView = [[UIScrollView alloc] initWithFrame:CGRectMake(10.0, 0.0, CGRectGetWidth(self.view.frame)-10.0, [self.tabHeight floatValue])];
...
//        [self.view insertSubview:self.tabsView atIndex:0];
    ...
    for (NSUInteger i = 0; i < self.tabCount; i++) {
        ...
        //add by jerry
        UIView *tabViewContent = [self.dataSource viewPager:self viewForTabAtIndex:i];
        UILabel *label = (UILabel *) tabViewContent;
        [label sizeToFit];
        
//        frame.size.width = label.frame.size.width+10;
        frame.size.width = tabViewContent.frame.size.width;
        tabView.frame = frame;
        
        frame = tabViewContent.frame;
        frame.origin.x = tabView.frame.origin.x;
        frame.origin.y = tabView.frame.origin.y;
        tabViewContent.frame = frame;
        NSLog(@"label width %@",NSStringFromCGRect(tabViewContent.frame));
        
        [self.tabsView addSubview:tabView];
        
        contentSizeWidth += CGRectGetWidth(tabView.frame)+20;
...
- (TabView *)tabViewAtIndex:(NSUInteger)index {
    ...
    if ([[self.tabs objectAtIndex:index] isEqual:[NSNull null]]) {
        ...
        tabViewContent.frame = tabView.frame;
        NSLog(@"tabViewContent width %@",NSStringFromCGRect(tabViewContent.frame));
