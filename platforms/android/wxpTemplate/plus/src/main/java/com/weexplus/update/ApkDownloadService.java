...
public class ApkDownloadService extends Service {
    ...
    public class LocalBinder extends Binder{
        ...
        public void setUpdateProgressListener(UpdateProgressListener listener){
            ApkDownloadService.this.setUpdateProgressListener(listener);
        }
    }

    public ApkDownloadService setSilent(boolean silent) {
        this.silent = silent;
        return this;
    }

    public ApkDownloadService setVersionCode(int code) {
    ...
    private static File getDownloadDir(ApkDownloadService service){
...
//            notification = new Notification.Builder(getApplicationContext(), YOUR_CHANNEL_ID).build();
    ...
    public boolean checkExistApk(Context c)
    {
       ...
       if(msg.contains(DOWNLOAD_APK_SPLIT))
       {
           ...
           int  appVersion=new AppMainfest(c).getVersionCode();
...
//        GetToast.useString(this,"asdfasdf");
    ...
    private static class DownloadApk extends AsyncTask<String, Integer, String>{
        ...
        private WeakReference<ApkDownloadService> updateServiceWeakReference;

        public DownloadApk(ApkDownloadService service){
            updateServiceWeakReference = new WeakReference<ApkDownloadService>(service);
        }

        @Override
        protected void onPreExecute() {
            super.onPreExecute();
            ApkDownloadService service = updateServiceWeakReference.get();
            if (service != null){
                service.start();
            }
        }
        ...
        protected String doInBackground(String... params) {
            DataCleanManager.deleteFilesByDirectory2(""+
                    ApkDownloadService.getDownloadDir(updateServiceWeakReference.get()) .getAbsolutePath()
            );
            final String downloadUrl = params[0];

            final File file = new File(ApkDownloadService.getDownloadDir(updateServiceWeakReference.get()),
                    ApkDownloadService.getSaveFileName(downloadUrl));
            if (DEBUG){
